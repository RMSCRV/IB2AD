...
def ActivationFunction(func):
    ...
    def relu(x):
        return x * (x > 0)
    def softmax(x):
        return np.exp(x - x.max(axis=1).reshape(x.shape[0],1))/np.sum(np.exp(x - x.max(axis=1).reshape(x.shape[0],1)))
    actfunc = {
        'logistic': expit,
        'identity': identity,
        'tanh': np.tanh,
...
def DerActivationFunction(func:str):
    ...
    def relu(x):
        return np.diag(x > 0)
    def softmax(x):
        y = np.exp(x - x.max())/np.sum(np.exp(x - x.max()))
        return np.dot(y.reshape(len(y),1), np.ones((1,len(y)), dtype=int)) * (np.identity(len(y)) - np.dot(np.ones((len(y),1), dtype=int), x.reshape(1,len(y))))
    deractfunc = {
        'logistic': sigmoid,
        'identity': identity,
        'tanh': tanh,
...
def SensAnalysisMLP(wts, bias, actfunc, X, y, 
    ...
    return self(sens, raw_sens, mlpstr, X, input_name, output_name)
...
class self:
    ...
    def sensitivityPlots(self, out=0):
        pltlist = []
        ...
        ax.vlines(x=0, ymin=0, ymax=max(sens['std']), color='blue')
